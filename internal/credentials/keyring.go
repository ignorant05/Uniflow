...
func Store(key, val string) error {
	if err := keyring.Set(constants.SERVICE, key, val); err != nil {
		return fmt.Errorf("failed to store credentials in keyring\nError: %w", err)
	}
...
func Get(key string) (string, error) {
	...
	if err != nil {
		if err == keyring.ErrNotFound {
			return "", fmt.Errorf("credential '%s' not found in keyring", key)
		}
		return "", fmt.Errorf("failed to retrieve credential '%s' from keyring", key)
	}
...
func Delete(key string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("failed to delete credential '%s' from keyring", key)
	}
