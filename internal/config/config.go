...
type Profile struct {
	...
	Gitlab  *GitlabConfig  `yaml:"gitlabci,omitempty" mapstructure:"gitlabci"`
}
...
func (cfg *Config) GetProfile(name string) (*Profile, error) {
	...
	if !exists {
		return nil, fmt.Errorf("no profile named %s registered, please re-check the username and try again", name)
	}
...
func (p *Profile) GetPlatform(platformName string) (interface{}, error) {
	...
	case strings.ToLower(constants.DEFAULT_CONFIG_PLATFORM):
		if p.Github == nil {
			return nil, fmt.Errorf("github configuration not found for this profile")
		}
	default:
