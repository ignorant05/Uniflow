...
func Load() (*Config, error) {
	...
	if _, err := os.Stat(configPath); os.IsNotExist(err) {
		return nil, fmt.Errorf("configuration file was not found at %s.\nPlease run: 'uniflow init' to create it", configPath)
	}
	...
	if err := v.ReadInConfig(); err != nil {
		return nil, fmt.Errorf("failed to read configuration file\nError: %w", err)
	}

	var cfg Config
	if err := v.Unmarshal(&cfg); err != nil {
		return nil, fmt.Errorf("failed to parse configuration file\nError: %w", err)
	}

	if err := resolveEnvVars(&cfg); err != nil {
		return nil, fmt.Errorf("failed to resolve environment variables\nError: %w", err)
	}
...
func Save(cfg *Config) error {
	...
	if err := v.WriteConfig(); err != nil {
		return fmt.Errorf("failed to save configuration file.\nError: %w", err)
	}
...
func Update(key, val string) error {
	...
	if len(parts) < 2 {
		return fmt.Errorf("invalid config key format. Use: profiles.<profile>.<platform>.<field>")
	}
...
func updatePlatformField(profile *Profile, platform, field, val string) error {
	...
	case constants.GITHUB:
		...
		default:
			return fmt.Errorf("unknown github field: %s", field)
		}
